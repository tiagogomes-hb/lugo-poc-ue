...
function createOptimizedPicture(
  ...
  breakpoints.forEach((br) => {
    ...
    source.setAttribute('srcset', `${absolute? origin: ''}${pathname}?width=${br.width}&format=webply&optimize=medium`);
  ...
  breakpoints.forEach((br, i) => {
    if (i < breakpoints.length - 1) {
      const source = document.createElement('source');
      if (br.media) source.setAttribute('media', br.media);
      source.setAttribute('srcset', `${absolute? origin: ''}${pathname}?width=${br.width}&format=${ext}&optimize=medium`);
      picture.appendChild(source);
    } else {
      ...
      img.setAttribute('src', `${absolute? origin: ''}${pathname}?width=${br.width}&format=${ext}&optimize=medium`);
    }
  });
